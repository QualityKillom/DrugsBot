...
namespace Domain.ValueObjects
{
    ...
    public abstract class BaseValueObject : IEquatable<BaseValueObject>
    {
        ...
        {   
            //используется как начальное для хэш-кода, чтобы уменьшить вероятность коллизий
            int hash = 17;
            //если значение свойства null, то используется 0, чтобы не было ошибок при вызове GetHashCode()
            //использование 31 помогает избежать коллизий хэш-кодов.
            foreach (var property in GetProperties())
            {
                var value = property.GetValue(this);
                hash = hash * 31 + (value?.GetHashCode() ?? 0);
            }
            // то же самое, что и предыдущий, но для полей объекта, используя метод GetFields()
            foreach (var field in GetFields())
            {
